...
if op_sys == 'Windows':
    dt_directory = 'C:\\Users\\kurt_\\PARAM DATA\\'
    _sl = '\\'
elif op_sys == 'Linux':
...
file=glob.glob(dt_directory+'old{}*.dat'.format(_sl))

for par in file:
    param_run = par[par.find('param'):par.find('.dat')]
...
#%% New Param
for kind in 'newpm comp'.split():
    file=glob.glob(f'{dt_directory}new{_sl}{kind}*.dat')
    print ('newpm directory:',dt_directory+'new{}*.dat'.format(_sl))
    for par in file:
        param_run = par[par.find(kind):par.find('.dat')]
        q = gl.getline(par, 2)
        q = q.split()
        qabs = [float(q[0])]
        qsca = [float(q[1])]
