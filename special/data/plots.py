...
if op_sys == 'Windows':
    dtdir = 'C:\\Users\\kurt_\\Google Drive\\Dropbox\\Code\\special\\data\\'
    _sl = '\\'
elif op_sys == 'Linux':
...
def linearize(data, sec_lin = False):
    ...
    b1 = 170
    b2 = 80
    data['m0'] = (abs(((a1*data.nr + np.log10(b1*data.ni)*1j)**2 -1)/((a2*data.nr + np.log10(b2*data.ni)*1j)**2 + 2)))**2
    if sec_lin:
...
# for p1 =  data['m0'] = (abs(((a1*data.nr + np.log10(b1*data.ni)*1j)**2 -1)/((a2*data.nr + np.log10(b2*data.ni)*1j)**2 + 2)))**2 a1=1, b1=0.85,a2=0.73, b2 = 18  ADJ R^2 = 8997 with exponential 2 terms
