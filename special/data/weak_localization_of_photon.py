# -*- coding: utf-8 -*-
"""
Created on Thu Jul 11 15:53:29 2019

@author: kurt_

Coherent Backscattering effect ( or oka weak localization of photon, quantum effect )
...
#                old_p22_err = (tm.P22*tm.P11 - old_pm.P22)[b1:b2]
                tm_pol = -tm.P12
                pm_pol = -pm.P12/pm.P11
                theta_maxpol_tm = tm.theta[tm_pol == max(tm_pol)].values[0]
                theta_maxpol_pm = pm.theta[pm_pol == max(pm_pol)].values[0]
                maxpol_tm = max(tm_pol)
                maxpol_pm = max(pm_pol)
                maxpol_pm_rw = max(-pm.P12_rw/pm.P11)
                m = complex(n_real, n_img)
                m0 = abs((m**2-1)/(m**2+2))

                pol_err = (tm_pol - pm_pol)[bs1:bs2]

                coeffs = L.fit(x, pol_err, deg=dg) # coeffs are mapped to [-1,1] window since L.P. are orthogonal in this domain
...
#                sw['corr_sca'] = corr_sca
                ...
                plotting = False
                if plotting:
                    # Correcting model by adding the Leg Fit
                    corr_pol = pd.Series(coeffs(x), index=x)
                    new_pol = pm_pol.copy()
                    new_pol = (pm_pol).add(corr_pol, fill_value=0)

                    fig, ax = plt.subplots(ncols=3,figsize=(24,6))
                    ax[0].plot(((new_pol))[bs1:bs2], label = 'new_pm python')
                    ax[0].plot( ((tm_pol))[bs1:bs2] , '.', label = 'tm')
                    ax[0].set_title('New P22 Python Correction'+runid)
                    ax[0].legend()
                    plt.grid()

                     # Old fit
                    ax[1].plot( ((pm_pol))[bs1:bs2], label = 'old_pm')
                    ax[1].plot( ((tm_pol))[bs1:bs2], '.', label = 'tm')
                    ax[1].legend()
                    ax[1].set_title('Old P22'+runid)
                    plt.grid()

                    # Error plots
                    ax[2].plot(x, pol_err , '.', label = 'p22_err')
                    ax[2].plot(x, corr_pol,'.', label = 'p22_corr' )
                    ax[2].plot(x, (tm_pol - new_pol)[bs1:bs2])
                    ax[2].legend()
...
Pol_coeeff = pd.DataFrame(dt)
Polcf_lt063 = Pol_coeeff[Pol_coeeff.alpha < 0.63]
Polcf_gt063 = Pol_coeeff[Pol_coeeff.alpha >= 0.63]
Polcf_gt047 = Pol_coeeff[Pol_coeeff.alpha >= 0.47]
Polcf_wl = Pol_coeeff[(Pol_coeeff.alpha >= 0.6) &y (Pol_coeeff.m0 * Pol_coeeff.alpha > 0.23) ]
