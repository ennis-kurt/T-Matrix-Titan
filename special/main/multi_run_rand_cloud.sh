...
for (( np=1;np<=${ncpu};np++));do
    ...
    echo "# This script runs a set of child processes on a single cpu core. 
        ...
        #child processes " > newscript${np}

    # Following first 6 lines may be unnecessary
    echo "#number of monomers " >> newscript${np}
    ...
    # echo z="("${b[@]}")" >>newscript${np}

    # ------------------------------------------------------------------------------------------------------------------------------------------

    # ------------------------------------------------------------------------------------------------------------------------------------------
    echo "for sn in ${ns[@]} ;do " >> newscript${np}
    echo "	for sx in ${xs[@]/./};do " >> newscript${np}
    echo "		for rn in ${nr[@]/./};do " >> newscript${np}
    echo "	   		for im in ${ni[@]/./};do " >> newscript${np}
    echo "	        	for sf in ${b[@]};do " >> newscript${np} #or "for dos in cat($ls);do ' ~/programs/mstmv3.0/run_mstm input/mstm/${sn}_${sx}_${rn}_${im}/$dos"
        ...
        echo '		            tmtf=${sf}${sn}${sx}${rn}${im}
                                ...
                                if [ "${Size}" -gt "3000" ]; then
                                    ...
                                    ./run_mstm $mstm_inp && echo "The run ${sf}_${sn}_${sx}_${rn}_${im} is completed on $(date)"# && rm tmatrixfiles/${tmtf}
                                fi
                            else 
                                ./run_mstm $mstm_inp && echo "The run ${sf}_${sn}_${sx}_${rn}_${im} is completed on $(date)" #&& rm tmatrixfiles/${tmtf}
...
if [ "$cpu_ids" != "" ]; then
	...
	for (( c=1; c<=${ncpu}; c++))
	{
        ...
        if [ $cpu -ne $(( ${cpu_ids[-1]} )) ]; then 
            taskset -c ${cpu} ./newscript${c} &  # if not the last run then run on background
        else
            taskset -c ${cpu} ./newscript${c} # Last run in a script allways run on foreground
        fi
	}
else
	...
	for (( c=1; c<=${ncpu}; c++))
	{
		...
		if [ $c -ne ${ncpu} ]; then
            ...
            ./newscript${c} &  # if not the last run then run on background # nice -n 10 
        else
            ./newscript${c}  # Last run in a script allways run on foreground #
        fi
	}
